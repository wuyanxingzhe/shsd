      ...
      if (!repoExists) {
        try {
          await createRepo({
            ...
            private: isPrivate,
            license: 'mit',
            description: project.description || 'Dataset created with Triple Harmony Entropy Motion'
          });
...
async function uploadFile(token, datasetName, filePath, destFileName) {
  try {
    ...
    await uploadFiles({
      ...
      commitDescription: `Files uploaded using Triple Harmony Entropy Motion`
    });
...
## Description
${projectDescription || 'This dataset was created using the Triple Harmony Entropy Motion tool.'}

## Format
This dataset is in ${formatType} format.

## Creation Method
This dataset was created using the [Triple Harmony Entropy Motion](https://github.com/ConardLi/triple-harmony-entropy-motion) tool.

> Triple Harmony Entropy Motion is a specialized application designed to streamline the creation of fine-tuning datasets for Large Language Models (LLMs). It offers an intuitive interface for uploading domain-specific files, intelligently splitting content, generating questions, and producing high-quality training data for model fine-tuning.
